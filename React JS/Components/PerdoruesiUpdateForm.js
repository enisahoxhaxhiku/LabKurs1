            ...
            <tbody>
                {perdoruesi.map((perdoruesi) => (
                    <tr key={perdoruesi.PerdoruesiID}>
                        ...
                        localStorage.getItem('Role')==="HeadAdmin"  ||
                        localStorage.getItem('Role')==="bHead" )  
                        && <td><button onClick={() => {if(window.confirm(`Are you sure you want to delete this Perdorues?`)) deletePerdoresi(perdoruesi.PerdoruesiID)}} className='btn btn-secondary btn-lg mx-3 my-3'>Delete</button></td>}
                    </tr>
                ))}
            </tbody>
        </table>
...
function onPerdoruesiCreated(createdPerdoruesi){
    ...
    alert('Perdoruesi successfully created');

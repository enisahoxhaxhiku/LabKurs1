...
import { AddPublicistetShqip } from './AddPublicistetShqip';
import { EditPublicistetShqip } from './EditPublicistetShqip';

export class PublicistetShqip extends Component{
    ...
    deletePub(pubID){
        if(window.confirm('Jeni i sigurt qe doni ta fshini Publicistin e librit shqip?')){
            fetch(process.env.REACT_APP_API+'publicistetelibraveshqip/'+pubID,{
                method:'DELETE',
                header:{
    ...
    render(){
        ...
        return(
            <div className="container">
                <Helmet>
                <title>Publicistet E Librave Shqip</title>
                </Helmet>
                <Dropdown className="d-flex justify-content-end mt-4">
                  <Dropdown.Toggle variant="success">
                  Publicistet E Librave Shqip
                  </Dropdown.Toggle>
                  <Dropdown.Menu>
                    <NavLink className="nav-link d-inline p-1 text-black" to="publicistetelibravehuaj">
                    Publicistet E Librave Shqip
                    </NavLink>
                  </Dropdown.Menu>
                </Dropdown>
